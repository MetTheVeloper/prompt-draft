...
import { getLayoutRegionVariableKey } from "~/utils/structuralVariables"
import type {
  LayoutRegion,
  LayoutRegionsState,
...
const { getVariableToken } = usePromptVariables()
...
function syncRegionDirtyState(item: VisualRegionDraft) {
  item.geometryDirty =
    item.isNew || !cellRectsEqual(item.rect, item.initialRect)
...
const canvasStyle = computed(() => {
  ...
  const baseStyle = {
    ...
    "--layout-grid-column-size": `${100 / grid.columns}%`,
    "--layout-grid-row-size": `${100 / grid.rows}%`,
  }
...
function duplicateRegion(source: VisualRegionDraft) {
  const nextColumn = Math.min(
    source.rect.column + 1,
    grid.columns - source.rect.columnSpan,
  )
  const nextRow = Math.min(
    source.rect.row + 1,
    grid.rows - source.rect.rowSpan,
  )
  const duplicatedRegion = createLayoutRegion(workingRegions.value.length, {
    ...cloneLayoutRegion(source.region),
    id: undefined,
    name: "",
    layer: workingRegions.value.length,
...
function hasRegionContentKey(region: LayoutRegion) {
  return Boolean(normalizeVariableKey(region.contentKey))
}

function regionContentKeyLabel(region: LayoutRegion) {
  const key = normalizeVariableKey(region.contentKey)

  if (key) {
    return getVariableToken(key)
  }

  return t(
    "modules.layout.fields.regions.visualBuilder.contentKey.empty",
  )
}


function assignRegionContentKey(
  item: VisualRegionDraft,
  ...
  item.region.contentKey = getVariableToken(key)
  item.metadataDirty = true
  syncRegionDirtyState(item)
}

function openRegionContentKeyPicker(item: VisualRegionDraft) {
  selectedRegionId.value = item.region.id

  openVariablePicker({
    excludeKeys: [
      "layout",
      getLayoutRegionVariableKey(item.region.id),
    ],
    insertOnSelect: false,
    closeOnSelect: true,
    onSelect: (variable) => {
      assignRegionContentKey(item, variable)
    },
  })
}
...
<template>
  <el-grid class="visual-layout-builder" :gap="12">
    ...
    <el-flex rules="csc" bg="background" class="visual-layout-builder__stage w100">
      <div
        ref="canvasRef"
        class="visual-layout-builder__canvas"
        :class="{
          'visual-layout-builder__canvas--draw': activeTool === 'draw',
          'visual-layout-builder__canvas--select': activeTool === 'select',
        }"
        :style="canvasStyle"
      ...
      >
        <div
          ...
          @keydown.enter.prevent="selectedRegionId = item.region.id"
          @keydown.space.prevent="selectedRegionId = item.region.id"
        >
          <el-flex
            rules="ccs"
            :gap="2"
            class="visual-layout-builder__region-content"
          >
            <el-text
              marker="blue50"
              color="white"
              :size="10"
              class="visual-layout-builder__region-label"
            >
            ...
            <el-text
              :size="8"
              :weight="600"
              :color="hasRegionContentKey(item.region) ? 'normal75' : 'normal45'"
              :title="t('modules.layout.fields.regions.visualBuilder.contentKey.select')"
              :class="[
                'visual-layout-builder__content-key crp',
                {
                  'is-empty': !hasRegionContentKey(item.region),
                },
              ]"
              @pointerdown.stop
              @click.stop="openRegionContentKeyPicker(item)"
            >
              {{ regionContentKeyLabel(item.region) }}
            </el-text>
          </el-flex>
          ...
          <el-flex
            ...
            rules="csc"
            :gap="2"
            class="visual-layout-builder__region-actions"
            @pointerdown.stop
            @click.stop
          >
            <el-button
            ...
            <el-button
              ...
              @click="removeRegion(item.region.id)"
            />
          </el-flex>
...
.visual-layout-builder__canvas::before {
  position: absolute;
  z-index: 1;
  inset: 0;
  content: "";
  ...
  background-size:
    var(--layout-grid-column-size)
    var(--layout-grid-row-size);
  pointer-events: none;
}

.visual-layout-builder__canvas--draw {
  cursor: crosshair;
}

.visual-layout-builder__canvas--select {
  cursor: default;
}
...
.visual-layout-builder__region-content {
  position: relative;
  z-index: 2;
...
.visual-layout-builder__content-key {
  max-width: 100%;
  padding: 1px 4px;
  overflow: hidden;
  border-radius: 4px;
  background: var(--themeSurface75);
  text-overflow: ellipsis;
  white-space: nowrap;
  pointer-events: auto;
  transition:
...
.visual-layout-builder__content-key:hover {
  background: var(--themeSurface);
  color: var(--themeBlue75);
}

.visual-layout-builder__content-key.is-empty {
  font-style: italic;
}

.visual-layout-builder__region-actions {
  position: absolute;
  z-index: 4;
  top: 4px;
  right: 4px;
