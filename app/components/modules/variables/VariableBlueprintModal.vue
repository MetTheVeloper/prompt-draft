...
function usedKeysForAllocation() {
  return Array.from(new Set([
    ...normalizedExternalKeys(),
    ...allDraftSlots().map((slot) => normalizeVariableKey(slot.key)).filter(Boolean),
  ]))
}
...
function groupKeysAtIndex(group: VariableBlueprintRepeatableGroup, index: number) {
  return group.slots
    .map((slot) => normalizeVariableKey(patternValue(slot.keyPattern, index)))
    .filter(Boolean)
}

function isGroupIndexAvailable(
  group: VariableBlueprintRepeatableGroup,
  index: number,
  usedKeys: Set<string>,
) {
  const keys = groupKeysAtIndex(group, index)
  if (keys.length !== group.slots.length) return false

  return keys.every((key) => {
    return isValidVariableKey(key) && !isReservedVariableKey(key) && !usedKeys.has(key)
  })
}

function nextAvailableGroupIndex(group: VariableBlueprintRepeatableGroup) {
  const usedKeys = new Set(usedKeysForAllocation())

  for (let index = 1; index <= 999; index += 1) {
    if (isGroupIndexAvailable(group, index, usedKeys)) return index
  }
...
function makeGroupSlot(
  slot: VariableBlueprintGroupSlot,
  semanticIndex: number,
): BlueprintDraftSlot {
  return {
    id: slot.id,
    enabled: !slot.optional,
    key: patternValue(slot.keyPattern, semanticIndex),
    value: patternValue(slot.valuePattern, semanticIndex),
    description: patternValue(slot.descriptionPattern, semanticIndex) || slot.id,
    type: slot.type,
    optional: slot.optional === true,
    typeEditable: slot.typeEditable === true,
  }
...
function addRepeatableGroupInstance(group: VariableBlueprintRepeatableGroup) {
  const semanticIndex = nextAvailableGroupIndex(group)

  draftGroups.push({
    id: `${group.id}:${semanticIndex}`,
    source: "repeatable",
    sourceGroupId: group.id,
    semanticIndex,
    label: `${group.label} ${semanticIndex}`,
    description: group.description,
    slots: group.slots.map((slot) => makeGroupSlot(slot, semanticIndex)),
  })
}

function repeatableGroupsFor(groupId: string) {
  return draftGroups.filter((group) => {
    return group.source === "repeatable" && group.sourceGroupId === groupId
  })
}

function setRepeatableGroupCount(
  group: VariableBlueprintRepeatableGroup,
  requestedCount: number,
) {
  const targetCount = Math.min(group.max, Math.max(group.min, requestedCount))
  let currentGroups = repeatableGroupsFor(group.id)

  while (currentGroups.length < targetCount) {
    addRepeatableGroupInstance(group)
    currentGroups = repeatableGroupsFor(group.id)
  }

  while (currentGroups.length > targetCount) {
    const last = currentGroups[currentGroups.length - 1]
    const index = draftGroups.findIndex((candidate) => candidate.id === last.id)
    if (index >= 0) draftGroups.splice(index, 1)
    currentGroups = repeatableGroupsFor(group.id)
  }

  groupCounts[group.id] = targetCount
}

function updateGroupCount(group: VariableBlueprintRepeatableGroup, value: ElDropdownValue) {
  const numeric = Number(value)
  if (!Number.isFinite(numeric)) return
  setRepeatableGroupCount(group, numeric)
}
...
function nextCustomIndex() {
  ...
  for (let index = 1; index <= 999; index += 1) {
    const key = normalizeVariableKey(patternValue(customSet.keyPattern, index))
    ...
    slots: [
      {
        ...
        enabled: true,
        key: patternValue(customSet.keyPattern, semanticIndex),
        value: "",
...
function initializeDraft() {
  ...
  props.blueprint.groups?.forEach((group) => {
    ...
    setRepeatableGroupCount(group, group.defaultCount)
  })
...
function slotIssue(slot: BlueprintDraftSlot, slotIndex: number) {
  ...
  const externalCollision = normalizedExternalKeys().includes(key)
  if (externalCollision) return "Variable key already exists"

  const enabledSlots = allDraftSlots().filter((candidate) => candidate.enabled)
  const duplicateIndex = enabledSlots.findIndex((candidate) => candidate === slot)
  const duplicate = enabledSlots.some((candidate, index) => {
    return index !== duplicateIndex && normalizeVariableKey(candidate.key) === key
  })

  if (duplicate) return "Duplicate variable key"
  return ""
}

const enabledSlots = computed(() => allDraftSlots().filter((slot) => slot.enabled))
const hasIssues = computed(() => {
  return allDraftSlots().some((slot, index) => !!slotIssue(slot, index))
})
const enabledCount = computed(() => enabledSlots.value.length)
...
function createBlueprintVariables() {
  if (hasIssues.value || !enabledSlots.value.length) return false

  const created: Array<Omit<PromptVariable, "id">> = enabledSlots.value.map((slot) => ({
    ...
    >
      <el-flex rules="ccs" :gap="2">
        ...
        <el-text :size="10" color="normal45">
          Choose how many complete {{ group.label.toLowerCase() }} profiles to create.
        </el-text>
      </el-flex>
      ...
      >
        ...
        <div class="variable-blueprint__slots">
          ...
          >
            ...
            <el-text
              v-if="slotIssue(slot, slotIndex)"
              :size="10"
              color="orange"
              icon="warning"
              icon-color="orange"
            >
              {{ slotIssue(slot, slotIndex) }}
            </el-text>
          </el-flex>
        </div>
      </el-flex>
