...
<template>
  <button
    ...
    <span class="wizard-entry-gateway__title">
      ...
      <span
        v-for="line in props.tail"
        :key="line"
        class="wizard-entry-gateway__tail">
        {{ line }}
      </span>
    </span>
...
.wizard-entry-gateway--left {
  transform-origin: left center;
}

.wizard-entry-gateway--right {
  transform-origin: right center;
}

.wizard-entry-gateway--active {
  transform: scale(1.012);
  background: radial-gradient(circle at 35% 50%, rgb(200 169 110 / 5%), transparent 64%);
}

.wizard-entry-gateway--right.wizard-entry-gateway--active {
  background: radial-gradient(circle at 65% 50%, rgb(120 145 188 / 4.5%), transparent 64%);
}

.wizard-entry-gateway--dimmed {
  opacity: 0.11;
}

.wizard-entry-gateway:focus-visible {
  outline: 1px solid color-mix(in srgb, var(--wizard-accent, #c8a96e) 72%, transparent);
  outline-offset: -8px;
}
