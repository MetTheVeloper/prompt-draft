...
test('public Blog detail enforces locale-aware 404, canonical redirect, shared article presentation and BlogPosting SEO', async () => {
  ...
  const blogMarkdown = await readFile('app/utils/publicBlogMarkdown.ts', 'utf8')
  ...
  assert.match(source, /<h1>\{\{ article\.title \}\}<\/h1>/)
  ...
  assert.match(blogMarkdown, /minimumHeadingLevel:\s*2/)
