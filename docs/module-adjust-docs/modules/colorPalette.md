...
Each built-in also carries exact hex swatches in source. Read `colorPaletteAssignmentsOptions` on current `main` when exact colors matter.

The shared assignment contract supports literal and variable-backed swatches. This is the preferred custom path when a desired color/palette is not built in.
...
Prefer structured palette assignments. For an unavailable color, use literal/custom swatches in the assignment editor when supported; do not replace the whole module with `customText` merely to introduce a non-preset color.