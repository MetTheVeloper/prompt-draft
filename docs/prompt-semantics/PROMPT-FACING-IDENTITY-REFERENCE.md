...
Status: **branch source of truth** for `refactor/module-wording`
...
Percentages communicate intended spatial proportions. They are not a claim of pixel-exact execution by an image-generation model.
...
| Effects | Generic named config | Semantic entity name/key; qualify only on collision |
