...
function validatePromotionBody(body) {
  const errors = []
  if (!body || typeof body !== 'object' || Array.isArray(body)) return [{ field: 'body', message: 'JSON body must be an object' }]
  if (!isUuid(body.sourceUserId)) errors.push({ field: 'sourceUserId', message: 'sourceUserId must be a UUID' })
  if (!parseDraftId(body.sourceDraftId)) errors.push({ field: 'sourceDraftId', message: 'sourceDraftId is invalid' })
  ...
  if (normalizeTelegramMessageId(body.telegramMessageId) === undefined) {
  ...
  const result = await queryDatabase(`
    SELECT
      ...
      COALESCE((
        ...
        WHERE media.user_id = drafts.user_id AND media.draft_id = drafts.draft_id
      ), '[]'::jsonb) AS images
...
async function audit(client, actor, action, metadata, targetUserId = null) {
  await client.query(`
...
async function createPromotedArchiveItem(actor, body) {
  ...
  await withDatabaseTransaction(async client => {
    const sourceResult = await client.query(`
      SELECT drafts.title, drafts.published_at AS "publishedAt"
      FROM prompt_drafts drafts
      INNER JOIN users ON users.id = drafts.user_id
      WHERE drafts.user_id = $1 AND drafts.draft_id = $2
        AND drafts.deleted_at IS NULL AND drafts.visibility = 'public'
        AND users.status = 'active'
    ...
    const inserted = await client.query(`
      INSERT INTO prompt_archive_items (
        ...
        optimized_for, variants, status, source_kind, source_user_id,
        source_draft_id, created_by, updated_by, created_at, updated_at
      ) VALUES (
        $1, COALESCE($2, nextval('prompt_archive_public_id_seq')), $2, $3, $4::jsonb, NULL,
        $5, $6, $7, $8, 'gpt-image-1', ARRAY['gpt-image-1']::text[],
        '[]'::jsonb, 'draft', 'user_draft', $9, $10, $11, $11, NOW(), NOW()
      )
    ...
    `, [
      ...
      telegramMessageId,
      channel,
      JSON.stringify({ en: body.title.en.trim(), fa: body.title.fa.trim() }),
      source.title,
    ...
    await audit(client, actor, 'archive.promote_user_draft', {
      archiveItemId,
      publicId,
      telegramMessageId,
...
async function moderateDeleteDraft(actor, sourceUserId, sourceDraftId) {
  let deletedAt = null
  await withDatabaseTransaction(async client => {
    const result = await client.query(`
      UPDATE prompt_drafts
      SET deleted_at = NOW(), server_updated_at = NOW()
      WHERE user_id = $1 AND draft_id = $2 AND deleted_at IS NULL
      RETURNING deleted_at AS "deletedAt", visibility, published_at AS "publishedAt"
    `, [sourceUserId, sourceDraftId])
    const row = result.rows[0]
    if (!row) {
      const error = new Error('Draft not found')
      error.code = 'MODERATION_DRAFT_NOT_FOUND'
      throw error
    }
    deletedAt = row.deletedAt.toISOString()
    await audit(client, actor, 'draft.moderation_delete', {
  ...
  return { id: sourceDraftId, userId: sourceUserId, deletedAt }
}
...
async function streamSourceImage({ response, corsHeaders, sourceUserId, sourceDraftId, imageId }) {
  ...
  const image = Array.isArray(source.images) ? source.images.find(candidate => candidate.id === imageId) : null
  if (!image?.storageKey) return false
  const storageResponse = await requestArchiveStorage({ method: 'GET', key: image.storageKey, config: getArchiveStorageConfig() })
...
export async function handleArchivePromotionRequest({ request, response, url, corsHeaders, sendJson, user }) {
  ...
  if (sourceMatch && request.method === 'DELETE') {
    if (!hasPermission(user, PERMISSIONS.DRAFTS_DELETE_ANY)) {
    ...
    try {
      sendJson(response, 200, { ok: true, draft: await moderateDeleteDraft(user, sourceUserId, sourceDraftId) }, corsHeaders)
    } catch (error) {
      if (error?.code === 'MODERATION_DRAFT_NOT_FOUND') {
        sendJson(response, 404, { ok: false, message: 'Draft not found' }, corsHeaders)
      } else {
        console.error('[Prompt Draft API] Draft moderation delete failed', error)
        sendJson(response, 500, { ok: false, message: 'Failed to moderate-delete Draft' }, corsHeaders)
      }
    }
    return true
  }
  ...
  if (sourceMatch) {
    ...
    try {
      ...
      if (!source) sendJson(response, 404, { ok: false, message: 'Public source Draft not found' }, corsHeaders)
      else sendJson(response, 200, { ok: true, draft: mapSourceDraft(source) }, corsHeaders)
    } catch (error) {
  ...
  if (imageMatch) {
    ...
    try {
      const streamed = await streamSourceImage({ response, corsHeaders, sourceUserId, sourceDraftId, imageId })
      if (!streamed) sendJson(response, 404, { ok: false, message: 'Source Draft image not found' }, corsHeaders)
    } catch (error) {
  ...
  try {
    sendJson(response, 201, { ok: true, item: await createPromotedArchiveItem(user, body) }, corsHeaders)
  } catch (error) {
    if (error?.code === '23505') {
