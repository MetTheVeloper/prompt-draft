...
function publicUrlToFilePath(urlPath) {
  ...
  if (!value.startsWith('/') || value.startsWith('//')) {
    throw new Error(`Legacy image path must be root-relative: ${value}`)
  }
  ...
  if (candidate !== publicRoot && !candidate.startsWith(rootPrefix)) {
    throw new Error(`Legacy image path escapes public root: ${value}`)
  }
  return candidate
}
...
function mirrorUrls(telegramMessageId, imageId) {
  const base = `/prompts/_snapshot/${telegramMessageId}/${imageId}`
  return {
    fullUrl: `${base}/full.webp`,
    thumbnailUrl: `${base}/thumb.webp`,
  }
}
...
async function normalizeImageForSnapshot(item, image, referencedMirrorDirs) {
  ...
  if (!Number.isInteger(position) || position < 0) {
    throw new Error(`Archive item ${item.telegramMessageId} has invalid image position`)
  }

  if (image.storageKey || image.thumbnailStorageKey) {
    const storageKey = requireNonEmptyString(image.storageKey, `Item ${item.telegramMessageId} full storage key`)
    const thumbnailStorageKey = requireNonEmptyString(
      image.thumbnailStorageKey,
      `Item ${item.telegramMessageId} thumbnail storage key`,
    )
    const imageId = requireNonEmptyString(image.id, `Item ${item.telegramMessageId} image id`)
    const mirrorDir = path.join(mirrorRoot, String(item.telegramMessageId), imageId)
    const urls = mirrorUrls(item.telegramMessageId, imageId)
  ...
  const sourcePath = requireNonEmptyString(
    image.sourcePath,
    `Item ${item.telegramMessageId} legacy source path`,
  )
  ...
  return {
    position,
    fullUrl: sourcePath,
    thumbnailUrl: sourcePath,
  }
}

async function loadAuthoritativeArchive(client) {
  const metadataResult = await client.query(`
    SELECT channel, model_history, imported_at
  ...
  const itemsResult = await client.query(`
    ...
    ORDER BY items.published_at DESC, items.telegram_message_id DESC
  `)
...
async function buildSnapshot(authoritative) {
  ...
  for (const row of authoritative.rows) {
    const telegramMessageId = Number(row.telegramMessageId)
    if (!Number.isSafeInteger(telegramMessageId) || telegramMessageId <= 0) {
      throw new Error(`Invalid Telegram message id in Archive: ${row.telegramMessageId}`)
    }
    if (!row.title?.en?.trim() || !row.title?.fa?.trim()) {
      throw new Error(`Archive item ${telegramMessageId} is missing localized titles`)
    }
    if (!Array.isArray(row.images)) throw new Error(`Archive item ${telegramMessageId} images are invalid`)

    const images = []
    for (const image of row.images) {
      images.push(await normalizeImageForSnapshot(row, image, referencedMirrorDirs))
    }
    images.sort((first, second) => first.position - second.position)
    images.forEach((image, index) => {
      if (image.position !== index) {
        throw new Error(`Archive item ${telegramMessageId} image positions are not contiguous from zero`)
      }
    })
    ...
    items.push({
      ...
      title: {
        en: row.title.en.trim(),
        fa: row.title.fa.trim(),
      },
      sourceTitle: typeof row.sourceTitle === 'string' ? row.sourceTitle : '',
      publishedAt: new Date(row.publishedAt).toISOString(),
      telegramUrl: requireNonEmptyString(row.telegramUrl, `Item ${telegramMessageId} Telegram URL`),
      model: {
        previewGeneratedWith: requireNonEmptyString(row.previewModel, `Item ${telegramMessageId} preview model`),
        optimizedFor: Array.isArray(row.optimizedFor) ? [...row.optimizedFor] : [],
      },
      images,
      prompt: requireNonEmptyString(row.prompt, `Item ${telegramMessageId} prompt`),
...
function buildParityReport(authoritative, payload) {
  ...
  if (payload.schemaVersion !== SNAPSHOT_SCHEMA_VERSION) {
    addMismatch(report, 'schemaVersion', 'archive', SNAPSHOT_SCHEMA_VERSION, payload.schemaVersion)
  }
  if (payload.channel !== authoritative.channel) {
    addMismatch(report, 'channel', 'archive', authoritative.channel, payload.channel)
  }
  if (payload.items.length !== authoritative.rows.length) {
    addMismatch(report, 'itemCount', 'archive', authoritative.rows.length, payload.items.length)
  }

  const rowsById = new Map(authoritative.rows.map((row) => [Number(row.telegramMessageId), row]))
  ...
  for (const item of payload.items) {
    ...
    if (new Date(item.publishedAt).getTime() !== new Date(row.publishedAt).getTime()) {
      addMismatch(report, 'publishedDates', item.id, row.publishedAt, item.publishedAt)
    }
    const expectedModel = {
      previewGeneratedWith: row.previewModel,
      optimizedFor: Array.isArray(row.optimizedFor) ? row.optimizedFor : [],
    }
    ...
    if (item.images.length !== row.images.length) {
      addMismatch(report, 'imageCounts', item.id, row.images.length, item.images.length)
    }
    const positions = item.images.map((image) => image.position)
    const expectedPositions = row.images.map((image) => Number(image.position))
    if (!sameJson(positions, expectedPositions)) {
      addMismatch(report, 'imageOrdering', item.id, expectedPositions, positions)
    }
  }
...
async function pruneStaleMirrors(referencedMirrorDirs) {
  let telegramEntries
  try {
    telegramEntries = await readdir(mirrorRoot, { withFileTypes: true })
  } catch (error) {
    if (error?.code === 'ENOENT') return
    throw error
  }

  for (const telegramEntry of telegramEntries) {
    if (!telegramEntry.isDirectory()) continue
    const telegramDir = path.join(mirrorRoot, telegramEntry.name)
    const imageEntries = await readdir(telegramDir, { withFileTypes: true })
    for (const imageEntry of imageEntries) {
      if (!imageEntry.isDirectory()) continue
      const imageDir = path.resolve(telegramDir, imageEntry.name)
      if (!referencedMirrorDirs.has(imageDir)) await rm(imageDir, { recursive: true, force: true })
    }
    const remaining = await readdir(telegramDir)
    if (!remaining.length) await rm(telegramDir, { recursive: true, force: true })
  }
}
