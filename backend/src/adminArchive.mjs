  ...
  try {
    ...
    for await (const chunk of request) {
      ...
      if (totalBytes > MAX_BODY_BYTES) {
        sendJson(response, 413, { ok: false, message: 'Archive request body is too large' }, corsHeaders)
        return null
      }
      chunks.push(chunk)
    }
...
function parseListQuery(url) {
  ...
  if (cursor === undefined) errors.push({ field: 'cursor', message: 'cursor must be a valid Manage Archive cursor' })

  const query = rawQuery?.trim() || ''
  if (query.length > MAX_QUERY_LENGTH) errors.push({ field: 'query', message: `query must be up to ${MAX_QUERY_LENGTH} characters` })
...
function normalizeOptionalTelegramMessageId(value) {
...
function validateArchiveInput(body) {
  const errors = []
  if (!isPlainObject(body)) return [{ field: 'body', message: 'JSON body must be an object' }]

  const telegramMessageId = normalizeOptionalTelegramMessageId(body.telegramMessageId)
  if (telegramMessageId === undefined) {
    errors.push({ field: 'telegramMessageId', message: 'Telegram message id must be empty or a positive integer' })
  }
  ...
  if (!isPlainObject(body.title)) {
    ...
    for (const locale of ['en', 'fa']) {
      ...
      if (!value || value.length > MAX_TITLE_LENGTH) {
        errors.push({ field: `title.${locale}`, message: `${locale.toUpperCase()} title must be 1-${MAX_TITLE_LENGTH} characters` })
      }
    }
  }
  ...
  if (
    ...
    errors.push({ field: 'sourceTitle', message: `sourceTitle must be a string up to ${MAX_SOURCE_TITLE_LENGTH} characters` })
  }

  if (typeof body.publishedAt !== 'string' || !body.publishedAt.trim() || Number.isNaN(Date.parse(body.publishedAt))) {
  ...
  if (
    !Array.isArray(body.tags) || body.tags.length > MAX_TAG_COUNT ||
    body.tags.some(tag => typeof tag !== 'string' || !tag.trim() || tag !== tag.trim() || tag !== tag.toLowerCase() || /\s/.test(tag)) ||
    new Set(body.tags).size !== body.tags.length
  ) {
    errors.push({ field: 'tags', message: `tags must contain up to ${MAX_TAG_COUNT} unique canonical tag slugs` })
  }
...
function normalizeArchiveInput(body) {
  return {
    ...
    tags: [...body.tags].sort((a, b) => a.localeCompare(b)),
  }
}
...
function mapSummaryRow(row) {
  return {
    ...
    telegramMessageId: nullableNumber(row.telegramMessageId),
    title: row.title,
...
function buildListWhere({ query, status, model, cursor }) {
  ...
  if (query) {
    ...
    conditions.push(`(
      POSITION(LOWER(${p}) IN LOWER(CAST(items.public_id AS TEXT))) > 0
      OR POSITION(LOWER(${p}) IN LOWER(COALESCE(CAST(items.telegram_message_id AS TEXT), ''))) > 0
      OR POSITION(LOWER(${p}) IN LOWER(COALESCE(items.titles->>'en', ''))) > 0
      OR POSITION(LOWER(${p}) IN LOWER(COALESCE(items.titles->>'fa', ''))) > 0
      OR POSITION(LOWER(${p}) IN LOWER(COALESCE(items.source_title, ''))) > 0
      OR POSITION(LOWER(${p}) IN LOWER(items.prompt)) > 0
      OR EXISTS (
        SELECT 1 FROM prompt_archive_item_tags sit
        INNER JOIN prompt_archive_tags st ON st.id = sit.tag_id
        WHERE sit.archive_item_id = items.id
          AND POSITION(LOWER(${p}) IN LOWER(st.slug)) > 0
      )
    )`)
  }
  if (status) { values.push(status); conditions.push(`items.status = $${values.length}`) }
  if (model) { values.push(model); conditions.push(`items.preview_model = $${values.length}`) }
  if (cursor) {
    values.push(cursor.updatedAt, cursor.id)
    conditions.push(`(items.updated_at, items.id) < ($${values.length - 1}::timestamptz, $${values.length}::uuid)`)
  }
  return { values, whereClause: conditions.length ? `WHERE ${conditions.join('\n      AND ')}` : '' }
}
...
async function listArchiveItems(params) {
  ...
  const result = await queryDatabase(`
    SELECT
      ...
      items.source_user_id AS "sourceUserId",
      items.source_draft_id AS "sourceDraftId",
      items.updated_at AS "updatedAt",
      to_char(items.updated_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS "cursorUpdatedAt",
      COALESCE((
  ...
  return {
    ...
    pageInfo: {
      hasMore,
      nextCursor: hasMore && last ? encodeCursor({ updatedAt: last.cursorUpdatedAt, id: last.id }) : null,
    },
  }
}
...
async function listCanonicalTags() {
  const result = await queryDatabase(`SELECT slug FROM prompt_archive_tags ORDER BY slug ASC`)
  return result.rows.map(row => row.slug)
}

async function requireKnownTags(client, tags) {
  if (!tags.length) return
  const result = await client.query(`SELECT slug FROM prompt_archive_tags WHERE slug = ANY($1::text[])`, [tags])
...
async function getArchiveChannel(client) {
  const result = await client.query(`SELECT channel FROM prompt_archive_metadata WHERE id = 1 LIMIT 1`)
...
function nextSourceKind(existingSourceKind) {
  return existingSourceKind === 'user_draft' ? 'user_draft' : 'managed'
}
...
async function updateArchiveItem(actor, id, input) {
  await withDatabaseTransaction(async client => {
    ...
    await requireKnownTags(client, input.tags)
    const channel = await getArchiveChannel(client)
    const sourceKind = nextSourceKind(existing.source_kind)
...
async function setArchiveStatus(actor, id, nextStatus) {
  await withDatabaseTransaction(async client => {
    ...
    const sourceKind = nextSourceKind(existing.source_kind)
    if (existing.status === nextStatus && existing.source_kind === sourceKind) return

    await client.query(`
      UPDATE prompt_archive_items
      SET status = $2, source_kind = $3, updated_by = $4, updated_at = NOW()
      WHERE id = $1
    `, [id, nextStatus, sourceKind, actor.id])

    const action = nextStatus === 'published' ? 'archive.publish' : nextStatus === 'archived' ? 'archive.archive' : 'archive.draft'
    await auditArchiveMutation(client, actor, action, {
      archiveItemId: id,
      telegramMessageId: nullableNumber(existing.telegram_message_id),
      fromStatus: existing.status,
      toStatus: nextStatus,
      fromSourceKind: existing.source_kind,
      toSourceKind: sourceKind,
...
function handleKnownMutationError(error, response, corsHeaders, sendJson) {
  if (error?.code === '23505') {
    sendJson(response, 409, { ok: false, message: 'Telegram message id or Archive public id already exists' }, corsHeaders)
    return true
  }
  if (error?.code === 'ARCHIVE_UNKNOWN_TAGS') {
    sendJson(response, 400, {
      ...
      errors: (error.tags || []).map(tag => ({ field: 'tags', message: `Unknown canonical tag: ${tag}` })),
    }, corsHeaders)
    return true
  }
...
export async function handleAdminArchiveRequest({ request, response, url, corsHeaders, sendJson, user }) {
  ...
  const statusMatch = url.pathname.match(/^\/api\/admin\/archive\/([0-9a-f-]{36})\/(draft|publish|archive)$/i)
  if (!collection && !tagsCollection && !detailMatch && !statusMatch) return false

  const isMutation = (collection && request.method === 'POST') || (detailMatch && request.method === 'PUT') || Boolean(statusMatch)
  const requiredPermission = isMutation ? PERMISSIONS.ARCHIVE_MANAGE : PERMISSIONS.ARCHIVE_VIEW
  ...
  if (collection && request.method === 'GET') {
    ...
    if (params.errors.length) {
      sendJson(response, 400, { ok: false, message: 'Invalid Manage Archive query', errors: params.errors }, corsHeaders)
      return true
    }
    try {
  ...
  if (collection && request.method === 'POST') {
    ...
    try {
      sendJson(response, 201, { ok: true, item: await createArchiveItem(user, normalizeArchiveInput(body)) }, corsHeaders)
    } catch (error) {
  ...
  if (detailMatch) {
    ...
    if (request.method === 'GET') {
      try {
        ...
        sendJson(response, item ? 200 : 404, item ? { ok: true, item } : { ok: false, message: 'Archive item not found' }, corsHeaders)
      } catch (error) {
    ...
    if (request.method === 'PUT') {
      ...
      try {
        sendJson(response, 200, { ok: true, item: await updateArchiveItem(user, id, normalizeArchiveInput(body)) }, corsHeaders)
      } catch (error) {
        if (handleKnownMutationError(error, response, corsHeaders, sendJson)) return true
  ...
  if (statusMatch) {
    ...
    try {
      sendJson(response, 200, { ok: true, item: await setArchiveStatus(user, id, nextStatus) }, corsHeaders)
    } catch (error) {
      if (handleKnownMutationError(error, response, corsHeaders, sendJson)) return true
