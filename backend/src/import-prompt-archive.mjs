...
function loadTrustedLocaleObject(source, filePath) {
  ...
  catch (error) { throw new Error(`Cannot evaluate trusted locale source ${filePath}: ${error.message}`) }
}
function readByPath(object, keyPath) { return keyPath.split('.').reduce((value, segment) => value?.[segment], object) }
...
function normalizeTags(tags, itemId) {
  ...
  if (tags.some((tag, index) => tag !== canonical[index])) throw new Error(`Item ${itemId} contains a non-canonical tag`)
...
function validateVariant(variant, itemId, index) {
  if (!variant || typeof variant !== 'object' || Array.isArray(variant)) throw new Error(`Item ${itemId} variant ${index} must be an object`)
...
function normalizeImages(value, itemId, normalizedSnapshot) {
  ...
  return value.map((image, index) => {
    if (!image || typeof image !== 'object' || Array.isArray(image)) throw new Error(`Item ${itemId}.images[${index}] must be an object`)
    ...
    const thumbnailUrl = typeof image.thumbnailUrl === 'string' && image.thumbnailUrl.trim() ? image.thumbnailUrl : fullUrl
    return { position, fullUrl, thumbnailUrl, normalizedSnapshot: true }
  })
}

function parseTelegramMessageId(telegramUrl, channel, fallbackId, normalizedSnapshot) {
  if (typeof telegramUrl === 'string' && telegramUrl.trim()) {
    const escaped = channel.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    const match = telegramUrl.trim().match(new RegExp(`^https://t\\.me/${escaped}/(\\d+)$`, 'i'))
    if (!match) throw new Error(`Item ${fallbackId}.telegramUrl does not match Archive channel`)
    const id = Number(match[1])
    if (!Number.isSafeInteger(id) || id <= 0) throw new Error(`Item ${fallbackId} has invalid Telegram message id`)
    return id
  }
  // Before normalized snapshots the public id was necessarily the Telegram id.
  return normalizedSnapshot ? null : fallbackId
}
...
function normalizeArchivePayload(payload, enMessages, faMessages) {
  ...
  const items = payload.items.map((item, itemIndex) => {
    ...
    const publicId = requireInteger(item.id, `items[${itemIndex}].id`)
    if (publicId <= 0) throw new Error(`Item ${publicId} id must be positive`)
    if (seenIds.has(publicId)) throw new Error(`Duplicate Archive public id ${publicId}`)
    ...
    let titleFa
    if (normalizedSnapshot) {
      titleEn = requireString(item.title?.en, `Item ${publicId}.title.en`)
      titleFa = requireString(item.title?.fa, `Item ${publicId}.title.fa`)
      titleKey = typeof item.titleKey === 'string' && item.titleKey.trim() ? item.titleKey : null
    } else {
      titleKey = requireString(item.titleKey, `Item ${publicId}.titleKey`)
    ...
    const publishedAt = requireString(item.publishedAt, `Item ${publicId}.publishedAt`)
    if (Number.isNaN(Date.parse(publishedAt))) throw new Error(`Item ${publicId}.publishedAt is not a valid date`)
    const telegramUrl = typeof item.telegramUrl === 'string' && item.telegramUrl.trim() ? item.telegramUrl.trim() : null
    const telegramMessageId = parseTelegramMessageId(telegramUrl, channel, publicId, normalizedSnapshot)
    ...
    return {
      ...
      publishedAt,
      telegramUrl,
      previewGeneratedWith: requireString(item.model?.previewGeneratedWith, `Item ${publicId}.model.previewGeneratedWith`),
      optimizedFor: requireStringArray(item.model?.optimizedFor, `Item ${publicId}.model.optimizedFor`),
      images: normalizeImages(item.images, publicId, normalizedSnapshot),
      prompt: requireString(item.prompt, `Item ${publicId}.prompt`),
      tags: normalizeTags(requireStringArray(item.tags, `Item ${publicId}.tags`), publicId),
      variants: (item.variants ?? []).map((variant, index) => validateVariant(variant, publicId, index)),
    }
  })
...
async function assertImporterOwnsSourceRows(client, source) {
  ...
  const result = await client.query(`
    SELECT public_id FROM prompt_archive_items
    WHERE source_kind IN ('managed', 'user_draft') AND public_id = ANY($1::int[])
    ORDER BY public_id LIMIT 20
  `, [source.items.map(item => item.publicId)])
  if (!result.rows.length) return
  const ids = result.rows.map(row => row.public_id).join(', ')
  throw new Error(
    `Archive bootstrap import is locked because snapshot rows are managed by the live Archive (public ids: ${ids}). ` +
  ...
  try {
    ...
    for (const slug of canonicalTags) {
      await client.query(`
        ...
        VALUES ($1, $2, 'legacy_json') ON CONFLICT (slug) DO NOTHING
      `, [randomUUID(), slug])
    }
    ...
    for (const item of source.items) {
      const itemResult = await client.query(`
        INSERT INTO prompt_archive_items (
          ...
          source_title, telegram_url, published_at, prompt, preview_model, optimized_for,
          variants, status, source_kind, updated_at
        ) VALUES (
          $1, $2, $3, $4, $5::jsonb, $6, $7, $8, $9, $10, $11, $12::text[],
          $13::jsonb, 'published', 'legacy_json', NOW()
        )
        ...
        RETURNING id
      `, [
        randomUUID(), item.publicId, item.telegramMessageId, source.channel, JSON.stringify(item.title),
        item.titleKey, item.sourceTitle, item.telegramUrl, item.publishedAt, item.prompt,
        item.previewGeneratedWith, item.optimizedFor, JSON.stringify(item.variants),
      ])
      ...
      await client.query('DELETE FROM prompt_archive_images WHERE archive_item_id = $1 AND position >= $2', [archiveItemId, item.images.length])
    }

    await client.query(`
      DELETE FROM prompt_archive_items
      WHERE source_kind = 'legacy_json' AND NOT (public_id = ANY($1::int[]))
    `, [source.items.map(item => item.publicId)])
    await client.query(`
...
async function buildParityReport(client, source) {
  const [metadataResult, itemsResult, tagsResult, imagesResult, catalogResult] = await Promise.all([
    ...
    client.query(`
      SELECT public_id, telegram_message_id, channel, titles, legacy_title_key, source_title,
             telegram_url, published_at, prompt, preview_model, optimized_for, variants
      FROM prompt_archive_items WHERE source_kind = 'legacy_json' ORDER BY public_id
    `),
    client.query(`
      SELECT i.public_id, t.slug FROM prompt_archive_item_tags it
      JOIN prompt_archive_items i ON i.id = it.archive_item_id
      JOIN prompt_archive_tags t ON t.id = it.tag_id
      WHERE i.source_kind = 'legacy_json' ORDER BY i.public_id, t.slug
    `),
    client.query(`
      SELECT i.public_id, pi.position, pi.source_path, pi.full_url, pi.thumbnail_url
      FROM prompt_archive_images pi JOIN prompt_archive_items i ON i.id = pi.archive_item_id
      WHERE i.source_kind = 'legacy_json' ORDER BY i.public_id, pi.position
    `),
    client.query(`
      SELECT DISTINCT t.slug FROM prompt_archive_tags t
      JOIN prompt_archive_item_tags it ON it.tag_id = t.id
      JOIN prompt_archive_items i ON i.id = it.archive_item_id
      WHERE i.source_kind = 'legacy_json' ORDER BY t.slug
    `),
  ])
  ...
  const expectedIds = source.items.map(item => item.publicId).sort((a, b) => a - b)
  const actualIds = itemsResult.rows.map(row => row.public_id).sort((a, b) => a - b)
  ...
  const rowsById = new Map(itemsResult.rows.map(row => [row.public_id, row]))
  const tagsById = new Map()
  for (const row of tagsResult.rows) {
    const values = tagsById.get(row.public_id) ?? []; values.push(row.slug); tagsById.set(row.public_id, values)
  }
  const imagesById = new Map()
  for (const row of imagesResult.rows) {
    const values = imagesById.get(row.public_id) ?? []; values.push(row); imagesById.set(row.public_id, values)
  }

  for (const item of source.items) {
    const row = rowsById.get(item.publicId)
    if (!row) continue
    if (row.telegram_message_id !== item.telegramMessageId) addMismatch(report, 'telegramIds', item.publicId, item.telegramMessageId, row.telegram_message_id)
    if (row.channel !== source.channel) addMismatch(report, 'channels', item.publicId, source.channel, row.channel)
    if (row.legacy_title_key !== item.titleKey) addMismatch(report, 'legacyTitleKeys', item.publicId, item.titleKey, row.legacy_title_key)
    if (row.titles?.en !== item.title.en) addMismatch(report, 'titles.en', item.publicId, item.title.en, row.titles?.en)
    if (row.titles?.fa !== item.title.fa) addMismatch(report, 'titles.fa', item.publicId, item.title.fa, row.titles?.fa)
    if (row.source_title !== item.sourceTitle) addMismatch(report, 'sourceTitles', item.publicId, item.sourceTitle, row.source_title)
    if (row.telegram_url !== item.telegramUrl) addMismatch(report, 'telegramUrls', item.publicId, item.telegramUrl, row.telegram_url)
    if (row.prompt !== item.prompt) addMismatch(report, 'promptBodies', item.publicId, item.prompt, row.prompt)
    if (new Date(row.published_at).getTime() !== new Date(item.publishedAt).getTime()) addMismatch(report, 'publishedDates', item.publicId, item.publishedAt, row.published_at)
    if (row.preview_model !== item.previewGeneratedWith || !sameJson(row.optimized_for, item.optimizedFor)) {
      addMismatch(report, 'modelFields', item.publicId,
    ...
    const actualTags = (tagsById.get(item.publicId) ?? []).slice().sort()
    if (!sameJson(expectedTags, actualTags)) addMismatch(report, 'tags', item.publicId, expectedTags, actualTags)
    if (!sameJson(row.variants, item.variants)) addMismatch(report, 'variants', item.publicId, item.variants, row.variants)

    const actualImages = imagesById.get(item.publicId) ?? []
    if (actualImages.length !== item.images.length) addMismatch(report, 'imageCounts', item.publicId, item.images.length, actualImages.length)
    const expectedPaths = item.images.map(image => image.fullUrl)
    const actualPaths = actualImages.map(image => image.source_path)
    if (!sameJson(expectedPaths, actualPaths)) addMismatch(report, 'imagePaths', item.publicId, expectedPaths, actualPaths)
    if (source.normalizedSnapshot) {
      ...
      if (!sameJson(expectedThumbs, actualThumbs)) addMismatch(report, 'thumbnailPaths', item.publicId, expectedThumbs, actualThumbs)
    }
  }
