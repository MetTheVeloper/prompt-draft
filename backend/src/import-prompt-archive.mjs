...
const sourceRoots = [
  process.env.ARCHIVE_SOURCE_ROOT,
  repositoryRoot,
  '/archive-source',
].filter(Boolean)
...
function normalizeArchivePayload(payload, enMessages, faMessages) {
  ...
  const items = payload.items.map((item, itemIndex) => {
    ...
    if (Number.isNaN(Date.parse(publishedAt))) {
      throw new Error(`Item ${telegramMessageId}.publishedAt is not a valid date`)
    }

    const previewGeneratedWith = requireString(
      item.model?.previewGeneratedWith,
      `Item ${telegramMessageId}.model.previewGeneratedWith`,
    )
    const optimizedFor = requireStringArray(
    ...
    const variants = (item.variants ?? []).map((variant, index) =>
      validateVariant(variant, telegramMessageId, index),
    )
...
async function importArchive(client, source) {
  ...
  try {
    await client.query(
      `
        INSERT INTO prompt_archive_metadata (
          id, schema_version, channel, source_updated_at, model_history, imported_at
        )
...
async function buildParityReport(client, source) {
  ...
  for (const row of tagsResult.rows) {
    const list = tagsByTelegramId.get(row.telegram_message_id) ?? []
    list.push(row.slug)
    tagsByTelegramId.set(row.telegram_message_id, list)
  }
...
try {
  ...
  const [payloadSource, enSource, faSource] = await Promise.all([
    ...
    readFile(faPath, 'utf8'),
  ])

  const payload = JSON.parse(payloadSource)
  const enMessages = loadTrustedLocaleObject(enSource, enPath)
  const faMessages = loadTrustedLocaleObject(faSource, faPath)
