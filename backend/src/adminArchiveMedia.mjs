    ...
    for await (const chunk of request) {
      ...
      if (totalBytes > MAX_BODY_BYTES) {
        sendJson(
          response,
          413,
          { ok: false, message: 'Archive image request body is too large' },
          corsHeaders,
        )
        return null
      }
...
function validateUploadBody(body) {
  ...
  const thumbnailWidth = requireDimension(
    thumbnail.width,
    'thumbnail.width',
    THUMBNAIL_MAX_EDGE,
  )
  const thumbnailHeight = requireDimension(
    thumbnail.height,
    'thumbnail.height',
    THUMBNAIL_MAX_EDGE,
  )
...
async function requireArchiveItem(id) {
  const result = await queryDatabase(
    `SELECT id, telegram_message_id FROM prompt_archive_items WHERE id = $1 LIMIT 1`,
    [id],
  )
  if (!result.rows[0]) {
    ...
    await withDatabaseTransaction(async (client) => {
      ...
      await client.query(
        ...
        [
          ...
          payload.width,
          payload.height,
          payload.thumbnailWidth,
          payload.thumbnailHeight,
          payload.fullBuffer.length,
          payload.thumbnailBuffer.length,
        ],
      )
      ...
      await client.query(
        `
          ...
          WHERE id = $1
        `,
        [archiveItemId, actor.id],
      )

      await auditArchiveMutation(client, actor, 'archive.image.upload', {
        archiveItemId,
        telegramMessageId: Number(lockedItem.telegram_message_id),
        imageId,
        position,
  ...
  await withDatabaseTransaction(async (client) => {
    ...
    const imageResult = await client.query(
      `
        ...
        FROM prompt_archive_images
        WHERE id = $1 AND archive_item_id = $2
        FOR UPDATE
      `,
    ...
    if (!deleted) {
      const error = new Error('Archive image not found')
      error.code = 'ARCHIVE_IMAGE_NOT_FOUND'
      throw error
    }
...
function handleKnownError(error, response, corsHeaders, sendJson) {
  ...
  if (error?.code === 'ARCHIVE_IMAGE_NOT_FOUND') {
    sendJson(response, 404, { ok: false, message: 'Archive image not found' }, corsHeaders)
    return true
  }
...
export async function handleAdminArchiveMediaRequest({
  request,
  response,
  url,
  corsHeaders,
  ...
  const collectionMatch = url.pathname.match(
    /^\/api\/admin\/archive\/([0-9a-f-]{36})\/images$/i,
  )
  const orderMatch = url.pathname.match(
    /^\/api\/admin\/archive\/([0-9a-f-]{36})\/images\/order$/i,
  )
  const imageMatch = url.pathname.match(
    /^\/api\/admin\/archive\/([0-9a-f-]{36})\/images\/([0-9a-f-]{36})$/i,
  )
  ...
  try {
    if (collectionMatch) {
      ...
      const image = await uploadArchiveImage(user, archiveItemId, payload)
      sendJson(response, 201, { ok: true, image }, corsHeaders)
      return true
    }
    ...
    if (imageMatch) {
      ...
      if (!isUuid(imageId)) {
        sendJson(response, 400, { ok: false, message: 'Invalid Archive image id' }, corsHeaders)
        return true
      }
      const result = await deleteArchiveImage(user, archiveItemId, imageId)
